...
class FinalHarness:
    ...
    def choose_focal(self, task: dict[str, Any], session: dict[str, Any], evidence: dict[str, Any]) -> dict[str, Any]:
        ...
        # 2) 대회 데이터의 route-binding trace는 최신 phase -> marker -> WM ref_code로
        #    focal을 일반화해 가리킨다. task_id별 정답표 없이 공개 record ontology만 해석한다.
        ...
        if isinstance(trace, dict) and isinstance(refs, dict):
            ...
            if ref:
                for obj in objects:
                    if str((obj.get("attrs") or {}).get("ref_code") or "") == str(ref):
                        return obj

        # 3) visible_history의 WM-code와 object ref_code가 맞으면 활용합니다.
    ...
    def infer_target(self, task: dict[str, Any], focal: dict[str, Any], session: dict[str, Any], evidence: dict[str, Any]) -> str:
        ...
        if isinstance(recall, dict):
            mem = self.memory.get(str(recall.get("memory_key") or ""), {})
            mclass = str(recall.get("memory_class") or "")
            if "standing" in mclass and mem.get("approval_channel"):
                return str(mem["approval_channel"])
            if "prior_result" in mclass and mem.get("last_success_target"):
...
if __name__ == "__main__":
    dev_payload = run_harness(dev_tasks, FinalHarness, harness_name="engineered_harness_dev") if dev_tasks else None
    ...
    submission_payload = run_harness(screening_tasks, FinalHarness, harness_name="engineered_harness")
